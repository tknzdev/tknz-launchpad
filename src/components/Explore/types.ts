...
export type TokenListTimeframe =
  (typeof TokenListTimeframe)[keyof typeof TokenListTimeframe];
const TokenListTimeframeRegex = new RegExp(
  `(?:${Object.values(TokenListTimeframe).join("|")})$`,
);
...
export function isTokenListBaseSortBy(
  sortBy: string,
): sortBy is TokenListBaseSortBy {
...
type TokenListTimeframeSortByPrefix =
  (typeof TokenListTimeframeSortByPrefix)[number];
export function isTokenListTimeframeSortByPrefix(
  sortBy: string,
): sortBy is TokenListTimeframeSortByPrefix {
  return TokenListTimeframeSortByPrefix.includes(
    sortBy as TokenListTimeframeSortByPrefix,
  );
}

type TokenListTimeframeSortBy =
  `${TokenListTimeframeSortByPrefix}${TokenListTimeframe}`;
export const TokenListTimeframeSortBy = TokenListTimeframeSortByPrefix.flatMap(
  (prefix) =>
    Object.values(TokenListTimeframe).map(
      (timeframe) => `${prefix}${timeframe}` as const,
    ),
);
export function isTokenListTimeframeSortBy(
  sortBy: string,
): sortBy is TokenListTimeframeSortBy {
...
export function normalizeSortByField(
  ...
  return isTokenListTimeframeSortBy(sortBy)
    ? stripTokenListTimeframeSortBy(sortBy)
    : sortBy;
}
function stripTokenListTimeframeSortBy(
  sortBy: TokenListTimeframeSortBy,
): TokenListTimeframeSortByPrefix {
  return sortBy.replace(
    TokenListTimeframeRegex,
    "",
  ) as TokenListTimeframeSortByPrefix;
}
...
export function resolveTokenListFilter(
