...
export const TokenChart: React.FC<ChartProps> = memo(
  ({ renderingId, style, opt }) => {
        ...
        try {
          ...
          widget.headerReady().then(() => {
            ...
            priceMcapTogglerRef.current?.addEventListener("click", () => {
              ...
              setChartConfig({
                lastInterval:
                  resolutionRef.current ?? DEFAULT_CHART_CONFIG.lastInterval,
          ...
          widget.onChartReady(() => {
            ...
            const foundVolumeStudy = studies.find(
            ...
            widget.applyOverrides({
              ...
              "mainSeriesProperties.statusViewStyle.symbolTextSource":
                "description", // display token symbol, jup.ag in chart
            });
    ...
    function updateButtonTitles(config: ChartConfig) {
      ...
      if (config.chartType === "mcap") {
        priceMcapTogglerRef.current.innerHTML =
          'Price / <span style="color:#c7f284">Mcap</span>';
      } else {
        priceMcapTogglerRef.current.innerHTML =
          '<span style="color:#c7f284">Price</span> / Mcap';
      }
    ...
    return (
      <>
        ...
        <div
          className={cn(
            "relative h-full w-full flex-1 overflow-hidden transition-all",
          )}
